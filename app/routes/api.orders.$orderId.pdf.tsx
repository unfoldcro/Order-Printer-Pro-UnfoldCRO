...
export const loader = async ({ request, params }: LoaderFunctionArgs) => {
  ...
  if (!template) {
    return new Response("No template found for type " + type, { status: 404 });
  }
  ...
  await db.log.create({
    data: {
      ...
      status: "success",
      message: `PDF generated for order ${orderData.name}`,
    },
  });
  ...
  return new Response(pdfBuffer as any, {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="${orderData.name.replace("#", "")}.pdf"`,
    },
  });
};
